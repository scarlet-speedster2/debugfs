...
class Ext2Filesystem(object):
  ...
  def checkIntegrity(self):
    ...
    if(len(self._superblock.copyLocations) > 1):
      ...
      bgtMembersEntries = list(map(dict, list(map(inspect.getmembers, firstBgtCopy.entries))))
      ...
      for groupId in self._superblock.copyLocations:
        ...
        try:
          ...
          bgtCopyMembersEntries = list(map(dict, list(map(inspect.getmembers, bgtCopy.entries))))
        except:
    ...
    inodesReachable = dict(list(zip(inodes, [False] * len(inodes))))
    blocks = self.__getUsedBlocks()
    blocksAccessedBy = dict(list(zip(blocks, [None] * len(blocks))))
